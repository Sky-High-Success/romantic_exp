...
class ulp_class {
	...
	function admin_warning() {
		echo '
		<div class="error"><p><strong>IMPORTANT!</strong> Please re-activate <strong>Layered Popups</strong> plugin!</p></div>';
	}
				...
				<table class="ulp_useroptions">
					<tr>
						...
						<td>
							...
							<br /><em>'.__('Disable OnLoad/OnExit/OnScroll popup if user subscribed through any popup or inline form.', 'ulp').'</em>
						</td>
					</tr>
					<tr>
						...
						<td>
							...
							<br /><em>'.__('If you turn this option on, only OnLoad/OnExit/OnScroll popup(s) will be loaded with website pages. If you disable this feature, all active popups will be loaded.', 'ulp').'</em>
						</td>
					</tr>
					<tr>
	...
	function admin_add_popup() {
		...
		echo '									
				...
				<h3>'.__('Subscription/Contact Form Parameters', 'ulp').'</h3>
				...
				<table class="ulp_useroptions">
					...
					<tr>
						...
						<td>
							...
							<br /><em>'.__('Enter the label for "Subscribe" button which appears on subscription form submission.', 'ulp').'</em>
						</td>
					</tr>';
		if ($this->options['fa_enable'] == 'on') {
			...
			echo '
							...
							<em>'.__('Select "Subscribe" button icon.', 'ulp').'</em>
						</td>
					</tr>';
		}
		echo '
					<tr>
						...
						<td>
							...
							<br /><em>'.__('Set the "Subscribe" button color.', 'ulp').'</em>
						</td>
					</tr>
					<tr>
						<th></th>
						<td>
							...
							<br /><em>'.__('Please tick checkbox to want to add color gradient to "Subscribe" button.', 'ulp').'</em>
						</td>
					</tr>
					<tr>
					...
					<tr>
						...
						<td style="vertical-align: middle;">
							...
							<br /><em>'.__('Set the border radius of "Subscribe" button.', 'ulp').'</em>
						</td>
					</tr>
					<tr>
						...
						<td style="vertical-align: middle;">
							...
							<br /><em>'.__('Customize CSS for "Subscribe" button.', 'ulp').'</em>
						</td>
					</tr>
					<tr>
						...
						<td style="vertical-align: middle;">
							...
							<br /><em>'.__('Customize CSS for "Subscribe" button when pointer is over the button.', 'ulp').'</em>
						</td>
					</tr>
					<tr>
...
<div id="ulp-layer-options-container" style="display: none;">
	...
	function save_popup() {
		...
		if (current_user_can('manage_options')) {
			...
			if (strlen($popup_options['button_label']) < 1) $errors[] = __('"Subscribe" button label is too short.', 'ulp');
			if (strlen($popup_options['button_color']) == 0 || $this->get_rgb($popup_options['button_color']) === false) $errors[] = __('"Subscribe" button color must be a valid value.', 'ulp');
			...
			if (strlen($popup_options['button_border_radius']) > 0 && $popup_options['button_border_radius'] != preg_replace('/[^0-9]/', '', $popup_options['button_border_radius'])) $errors[] = __('Invalid "Subscribe" button border radius.', 'ulp');
...
<body>';
			...
			<td>'.(empty($field['value']) ? '-' : str_replace(array("\r", "\n"), array('', '<br />'), esc_html($field['value']))).'</td>
				...
				case 'ulp-export-full':
					...
					if (!empty($popup_details)) {
						...
						$popup_options = unserialize($popup_details['options']);
						if (is_array($popup_options)) $popup_options = array_intersect_key($popup_options, $this->default_popup_options);
						else $popup_options = $this->default_popup_options;
	...
	function get_popups($_popup_id = '', $_add_overlay = true) {
		...
		if (!empty($layer_webfonts)) {
			...
			$webfonts_array = json_decode($this->google_fonts, true);
			$used_webfonts = array();
			foreach ($webfonts_array['items'] as $webfont) {
		...
		//if ($this->check_options() === true) {
			if (isset($_atts['id'])) {
				...
				if ($popup) {
					...
					$style .= '#ulp-inline-'.$inline_id.' .ulp-input,#ulp-inline-'.$inline_id.' .ulp-input:hover,#ulp-inline-'.$inline_id.' .ulp-input:active,#ulp-inline-'.$inline_id.' .ulp-input:focus{border-width: '.intval($popup_options['input_border_width']).'px !important; border-radius: '.intval($popup_options['input_border_radius']).'px !important; border-color:'.(empty($popup_options['input_border_color']) ? 'transparent' : $popup_options['input_border_color']).';background-color:'.(empty($popup_options['input_background_color']) ? 'transparent' : $popup_options['input_background_color']).' !important;background-color:'.(empty($popup_options['input_background_color']) ? 'transparent' : 'rgba('.$bg_color['r'].','.$bg_color['g'].','.$bg_color['b'].','.floatval($popup_options['input_background_opacity'])).') !important;'.(!empty($popup_options['input_css']) ? $popup_options['input_css'] : '').'}';
	...
	function subscribe() {
		...
		$custom_fields = array_merge($custom_fields, array('ip' => array('name' => 'IP Address', 'value' => $_SERVER['REMOTE_ADDR']), 'agent' => array('name' => 'User Agent', 'value' => $_SERVER['HTTP_USER_AGENT']), 'url' => array('name' => 'URL', 'value' => $_SERVER['HTTP_REFERER'])));
		...
		$return_data = array();
		$return_data['status'] = 'OK';
		$return_data['return_url'] = strtr($popup_options['return_url'], $urlencoded);
